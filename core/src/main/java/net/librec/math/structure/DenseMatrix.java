...
public class DenseMatrix implements DataMatrix, Serializable {
    ...
    //**************************************************************************
    //This is the randomizer that was used in the python code.
    public void initRand() {
        for (int i = 0; i < numRows; i++) {
            for (int j = 0; j < numColumns; j++) {
                data[i][j] = 0.1 * (Randoms.random() - 0.5);
            }
        }
    }
     ...
     *
     * @param rowId row id
     * @param deep whether to copy data or only shallow copy for executing
     * speedup purpose
     * @return a vector of a specific row
     */
     ...
     * than {@code inverse}, especially when {@code numRows} is large.
     *
     * @return the inverse matrix of current matrix
     */
    ...
    /**
     * @return Moore–Penrose pseudoinverse based on singular value decomposition
     * (SVD)
     *
